...
func main() {
	...
	for {
		go func() {
			...
			queryInput := TableWithBitInt64{
				TypeID: int(typeID),
				Value:  value,
			}
			var queryOutput TableWithBitInt64
			...
			if err != nil && err != sql.ErrNoRows {
				...
				strBit := strconv.FormatInt(queryOutput.Source, 2)
